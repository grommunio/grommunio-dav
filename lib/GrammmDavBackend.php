...
class GrammmDavBackend {
    ...
    public function GetObjects($id, $fileExtension, $filters = array()) {
        ...
        $rows = mapi_table_queryallrows($table, array(PR_SOURCE_KEY, PR_LAST_MODIFICATION_TIME, PR_MESSAGE_SIZE, $properties['goid']));
    ...
    public function CreateObject($folderId, $folder, $objectId) {
        ...
        mapi_setprops($mapimessage, array($properties['goid'] => $goid));
        return $mapimessage;
    }
    ...
    public function GetIdOfMapiMessage($folderId, $mapimessage) {
        ...
        // - PR_SOURCE_KEY
        $props = mapi_getprops($mapimessage, array($properties['goid'], PR_SOURCE_KEY));
