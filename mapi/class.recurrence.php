		...
		*/
		...
		function saveRecurrencePattern()
		{
			...
			if ($term == 0x23) {
				...
				if ($occTimeRange) {
					if ($occSingleDayRank)
						$pattern = sprintf(dngettext("kopano","Occurs every %s effective %s for %s occurrence from %s to %s.",
								 "Occurs every %s effective %s for %s occurrences from %s to %s.", $numocc), $type, $start, $numocc, $startocc, $endocc);
					else
						$pattern = sprintf(dngettext("kopano","Occurs every %s %s effective %s for %s occurrence from %s to %s.",
								 "Occurs every %s %s effective %s for %s occurrences %s to %s.", $numocc), $everyn, $type, $start, $numocc, $startocc, $endocc);
				} else {
					if ($occSingleDayRank)
						$pattern = sprintf(dngettext("kopano","Occurs every %s effective %s for %s occurrence.",
								 "Occurs every %s effective %s for %s occurrences.", $numocc), $type, $start, $numocc);
					else
						$pattern = sprintf(dngettext("kopano","Occurs every %s %s effective %s for %s occurrence.",
